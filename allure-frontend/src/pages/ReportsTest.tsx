...
/* ---------- «закреплённый» UUID ---------- */
const MAIN_UUID = import.meta.env.VITE_MAIN_TEST_UUID as string;
...
export default function ReportsTest() {
  ...
  /** Загружаем ВСЕ отчёты → фильтруем test → сортируем */
  const loadRows = useCallback(() => {
    setLoad(true);
    fetchJSON<Report[]>('/api/reports')
      .then(all =>
        all
          .filter(r => r.path.toLowerCase().includes('test'))
          .sort((a, b) => {
