...
/* ---------- «закреплённый» UUID ---------- */
const MAIN_UUID = import.meta.env.VITE_MAIN_DEV_UUID as string;
...
/* ===================== Компонент ==================== */
export default function ReportsDev() {
  ...
  /** Загружаем ВСЕ отчёты → фильтруем dev → сортируем */
  const loadRows = useCallback(() => {
    setLoad(true);
    fetchJSON<Report[]>('/api/reports')
      .then(all =>
        all
          .filter(r => r.path.toLowerCase().includes('dev'))
          .sort((a, b) => {
