/* -----------------------------------------------------------------------
 *  src/pages/ReportsPreprod.tsx
 *  Страница «Отчёты Preprod» (React + MUI X)
 * --------------------------------------------------------------------- */
...
dayjs.extend(utc);                                // ← подключаем UTC-плагин
...
/* ---------- «закреплённый» UUID ---------- */
const MAIN_UUID = import.meta.env.VITE_MAIN_PREPROD_UUID as string;

/* ---------- helper для сокращения URL ---------- */
const shorten = (href: string): string => {
  try {
    const u = new URL(href);
    return u.pathname.replace(/^\/+/, '');    // без ведущего /
  } catch {                                   // если href невалидный
    return href;
  }
};

/* ===================== Компонент ==================== */
export default function ReportsPreprod() {
  ...
  /** Загружаем ВСЕ отчёты → фильтруем preprod → сортируем */
  const loadRows = useCallback(() => {
    setLoad(true);
    fetchJSON<Report[]>('/api/reports')
      .then(all =>
        all
          .filter(r => r.path.toLowerCase().includes('preprod'))
          .sort((a, b) => {
            if (a.uuid === MAIN_UUID) return -1;
            if (b.uuid === MAIN_UUID) return  1;
            return (
  ...
  const columns: GridColDef[] = useMemo(() => {
    const createdCol: GridColDef = {
      field: 'createdDateTime',
      headerName: 'Created',
      width: 140,
      valueFormatter: ({ value }) =>
        dayjs.utc(value as string).local().format('DD-MM-YYYY HH:mm'),
    };

    const urlCol: GridColDef = {
      field: 'url',
      headerName: 'Url',
      flex: 1,
      minWidth: 200,
      ...
      renderCell: ({ row }) => {
        ...
        return (
          <a href={`/allure/reports/${uuid}/index.html`}
    ...
    return [createdCol, urlCol, pipelineCol];
  }, [isAdmin]);
  ...
  const handleUpload = async () => {
    ...
    form.append('allureReportArchive', file);

    const reportPath = encodeURIComponent(file.name.replace(/\.zip$/i, ''));
    try {
    ...
    <>
      ...
      <AppBar position="fixed"
        ...
        <Toolbar sx={{ minHeight: 56, px: 3 }}>
          ...
          <Box sx={{ flexGrow: 1 }} />
          <Box sx={{
            px: isMobile ? 2 : 3, py: isMobile ? 0.75 : 1,
            bgcolor: '#E6E7EB', color: '#2E3034', borderRadius: 2,
            fontWeight: 600, '&:hover': { bgcolor: '#DADBDF' },
          }}>
            <BccButton
              ...
              style={{ background: 'transparent', border: 0, padding: 0,
                       minWidth: 0, color: 'inherit', font: 'inherit' }}>
