...
public class ResultUploadDialog extends Dialog { //NOPMD
    ...
    public ResultUploadDialog(
            BiFunction<MemoryBuffer, String, Object> uploadConsumer,
        ...
        Label dropLabel = new Label(format("Upload allure {} as Zip archive (.zip)", type));
        //dropLabel.getStyle().set("color", "white"); // Устанавливаем белый цвет текста
        ...
        upload.addSucceededListener(event -> {
            try {
                var uploadResponse = uploadConsumer.apply(buffer, event.getFileName());
                show(info(format(
                                "File '{}- {} bytes' loaded: {}",
