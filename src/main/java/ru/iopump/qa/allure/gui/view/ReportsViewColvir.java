...
@Tag("reports-view-colvir")
@PageTitle("ReportsColvir | " + ALLURE_SERVER)
@Route(value = "reports-colvir", layout = ReportsMain.class)

@Slf4j
public class ReportsViewColvir extends VerticalLayout {
    ...
    public ReportsViewColvir(final JpaReportService jpaReportService,
    ...
    private static ListDataProvider<ReportEntity> asProvider(final JpaReportService jpaReportService,ReportProperties reportProperties) {
        ...
        List<ReportEntity> sortedList = collection.stream()
                .sorted(Comparator.comparing(reportEntity -> {
                    if (reportEntity.getUuid().equals(UUID.fromString(reportProperties.getYourColvirReportProperty()))) {
    ...
    public void postConstruct() {
        reports.applyPathFilter("colvir");
