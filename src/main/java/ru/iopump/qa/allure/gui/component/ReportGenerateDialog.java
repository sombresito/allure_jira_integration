...
public class ReportGenerateDialog extends Dialog {
    ...
    private final Button okButton  = new Button("Ok", e -> onClickCloseAndDiscard());
    ...
    private HorizontalLayout createButtonsLayout() {
        ...
        generate.addClassName("generate-button");

        okButton.addClassName("green-button");
        okButton.addClickShortcut(Key.ESCAPE);
        okButton.addClickListener(e -> onClickCloseAndDiscard());

        close.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
        close.addClickShortcut(Key.ESCAPE);
        close.addClickListener(e -> onClickCloseAndDiscard());
        close.addClassName("cancel-button");

        return new HorizontalLayout(generate, okButton, close);
    }
    ...
    private void configureDialog() {
        ...
        var title = new H3("Создание отчета");
        addClassName("custom-dialog-background");
        getElement().getStyle().set("padding", "20px"); // добавление отступа для эстетики
        add(title, form, info, error);
    }

    public static class FormPayload implements Closeable {
        @PropertyId("resultUuid")
        private final TextField resultUuid = new TextField("ID");
        @PropertyId("path")
        ...
        @PropertyId("logsUrl")
        private final TextField logsUrl = new TextField("Logs Url");
        ...
        public ReportGenerateRequest toReportGenerateRequest(GenerateDto generateDto) {
            ...
            request.setLogsUrl(generateDto.getLogsUrl());
        ...
        private void visitForm(final FormLayout form) {
            form.add(resultUuid, path, build, jiraIssueKey, logsUrl, deleteResults);
        }
