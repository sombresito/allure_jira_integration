...
@Tag("reports-view-dev")
@PageTitle("ReportsDev | " + ALLURE_SERVER)
@Route(value = "reports-dev ", layout = ReportsMain.class)
@Slf4j
public class ReportsViewDev extends VerticalLayout {
    ...
    public ReportsViewDev(final JpaReportService jpaReportService,
    ...
    private static ListDataProvider<ReportEntity> asProvider(final JpaReportService jpaReportService,ReportProperties reportProperties) {
        ...
        List<ReportEntity> sortedList = collection.stream()
                .sorted(Comparator.comparing(reportEntity -> {
                    if (reportEntity.getUuid().equals(UUID.fromString(reportProperties.getYourDevReportProperty()))) {
    ...
    public void postConstruct() {
        reports.applyPathFilter("dev","дев");
        ...
        String currentPageKeyword = "dev"; // Задайте значение "cfg", "test" или "dev" в зависимости от текущей страницы
