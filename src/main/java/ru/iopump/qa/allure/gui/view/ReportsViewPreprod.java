...
@Tag("reports-view-preprod")
@PageTitle("ReportsPreprod | " + ALLURE_SERVER)
@Route(value = "reports-preprod", layout = ReportsMain.class)
@Slf4j
public class ReportsViewPreprod extends VerticalLayout {
    ...
    public ReportsViewPreprod(final JpaReportService jpaReportService,
    ...
    private static ListDataProvider<ReportEntity> asProvider(final JpaReportService jpaReportService,ReportProperties reportProperties) {
        ...
        List<ReportEntity> sortedList = collection.stream()
                .sorted(Comparator.comparing(reportEntity -> {
                    if (reportEntity.getUuid().equals(UUID.fromString(reportProperties.getYourPreprodReportProperty()))) {
    ...
    public void postConstruct() {
        reports.applyPathFilter("preprod");
        ...
        String currentPageKeyword = "preprod"; // Задайте значение "cfg", "test" или "dev" в зависимости от текущей страницы
