...
@Tag("reports-view-test")
@PageTitle("ReportsTest | " + ALLURE_SERVER)
@Route(value = "reports-test", layout = ReportsMain.class)
@Slf4j
public class ReportsViewTest extends VerticalLayout {
    ...
    public ReportsViewTest(final JpaReportService jpaReportService,
    ...
    private static ListDataProvider<ReportEntity> asProvider(final JpaReportService jpaReportService,ReportProperties reportProperties) {
        ...
        List<ReportEntity> sortedList = collection.stream()
                .sorted(Comparator.comparing(reportEntity -> {
                    if (reportEntity.getUuid().equals(UUID.fromString(reportProperties.getYourTestReportProperty()))) {
    ...
    public void postConstruct() {
        reports.applyPathFilter("тест", "test");
        ...
        String currentPageKeyword = "test"; // Задайте значение "cfg", "test" или "dev" в зависимости от текущей страницы
